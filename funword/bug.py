...
segmenter = OrderedPhraseSegmenter(MAX_PHRASE_LENGTH)
...
# 2. 生成原始与每个unit删一次后的clip特征
pipe = StableDiffusionPipeline.from_pretrained(
    "runwayml/stable-diffusion-v1-5", torch_dtype=torch.float16 if device == "cuda" else torch.float32, use_safetensors=True
).to(device)
pipe.enable_attention_slicing()
generator = torch.Generator(device=device).manual_seed(42)
base_latents = torch.randn((1, 4, 64, 64), generator=generator, device=device, dtype=pipe.unet.dtype)

def generate_image(prompt, base_latents, pipe):
    return pipe(prompt=prompt, latents=base_latents.clone(), num_inference_steps=25, guidance_scale=7.5, height=512, width=512).images[0]
def get_clip_feature(image, preprocess_clip, model_clip, device):
    with torch.no_grad():
        ...
        return image_feature.cpu().numpy().flatten()
...
unit_del_images, unit_del_features = [], []
...
plt.close()
print("特征分布已保存为 clip_unit_del_scatter.png")
